 ...
 *
 * @param {{ id?: number, deadline_days: number }[]} prizes
 * @param {number} covers
 * @param {number} gifts
 * @returns {{
 *   winWeight: number,
 ...
 *   realWeights: number[],
 *   loseWeights: number[],
 *   updates: { id: number, probability: number, deadline_days: number }[]
 * }}
 */
function calibratePrizeWeights(prizes, covers, gifts) {
  ...
  const updates = [
    ...real.map((p, i) => ({
      id: p.id,
      probability: realWeights[i] ?? 0,
      deadline_days: Number(p.deadline_days) || 0,
    })),
    ...lose.map((p, i) => ({
      id: p.id,
      probability: loseWeights[i] ?? 0,
      deadline_days: Number(p.deadline_days) || 0,
    })),
  ];

  return { winWeight, loseWeight, realWeights, loseWeights, updates };
}
